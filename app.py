...
def init_db():
    ...
    # context predict pe zi (folosit pt atribuirea automată)
    cur.execute("""
...
def get_predict_context_for(date_iso: str) -> Optional[dict]:
    """Întoarce contextul pentru data indicată; dacă nu există, caută ziua anterioară."""
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT date, driver, route, vehicle, stops FROM predict_context WHERE date=?", (date_iso,))
    row = cur.fetchone()
    if not row:
        # încearcă ziua anterioară (cazul bon motorină „cu o zi în urmă”)
