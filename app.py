...
def main():
    ...
    with st.sidebar:
        ...
        st.info("💡 Bonurile de motorină sunt adesea pentru **ziua precedentă** – le mapez automat la Predict.")

    st.subheader("1) Încarcă fișiere (jpg/png/pdf/xls/xlsx/csv) – procesare automată")
    uploads = st.file_uploader(
        "Urcă documente (detectez Predict & Motorină și salvez automat)",
    ...
    def save_file_and_get_path(up, subdir: Path) -> Path:
    ...
    if uploads:
        ...
        for up in uploads:
            try:
                ...
                # OCR la imagini/PDF (nu oprește aplicația dacă lipsește tesseract)
                ...
                if ext in [".xlsx",".xls",".csv"]:
                    try:
                        if ext == ".csv":
                            ...
                            df_x = pd.read_excel(io.BytesIO(up.getbuffer()))
                    except Exception as e:
                        st.error(f"Nu pot citi {up.name}: {e}")
                        continue
                    ...
                    processed_summary.append({"fisier": up.name, "tip": detected_type, "rows_saved": added_rows})

                # IMAGINE/PDF – bon motorină (Menge)
                if text and detected_type == "":
                    ...
                    if liters is not None and liters > 0:
                        ...
                        auto_inserts += 1
                        processed_summary.append({
                            "fisier": up.name, "tip": "FUEL_OCR",
                            "date": date_iso, "driver": row["driver"], "route": row["route"],
                            "fuel_l": liters, "fuel_cost": row["fuel_cost"]
                        })

            except Exception as e:
                st.error(f"❌ Eroare la procesarea fișierului {up.name}: {e}")

        # Excel live – nu bloca aplicația dacă scrierea eșuează
        try:
            write_master_excel()
        except Exception as e:
    ...
    try:
        ...
        st.error("❌ Eroare în secțiunea de statistici:")
        st.exception(e)
...
if __name__ == "__main__":
    try:
        ...
        st.error("❌ Eroare critică la inițializare/rulare.")
        st.exception(e)
